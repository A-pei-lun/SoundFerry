...
### ✅ v0.9（当前 · MVP）
离线语音转写 + 翻译的基础能力打通，核心链路跑通。

### 🔜 v1.0 — 多引擎 + 能力补全
- [ ] **Sherpa-ONNX 引擎集成** — 支持 Whisper、SenseVoice 等更多模型格式
- [ ] **多引擎切换 UI** — 引擎选择界面完善，切换模型不出错
- [ ] **翻译开关真正可用** — 目前灰掉的功能补全
- [ ] **英文 ASR 体验优化** — 切换英文模型后自动匹配文本处理规则
- [ ] **录音波形可视化** — 录音时显示音频波形反馈
- [ ] **翻译结果自动填充** — 停止录音后自动填入历史记录
- [ ] **降低 APK 体积** — 大模型改为首次启动时下载
...
- [ ] **多语言支持** — 日/韩/法/德等多语种互译
- [ ] **录音文件导入** — 导入已有音频文件进行转写
- [ ] **导出功能** — 将转写结果导出为 TXT / SRT / 字幕格式
- [ ] **蓝牙耳机支持** — 通过蓝牙耳机麦克风录音
- [ ] **主题在线商店** — 用户上传/下载自定义主题
- [ ] **桌面 Widget** — 主屏幕快捷录音

> 路线图会根据实际反馈调整，你有想法直接提 Issue。
